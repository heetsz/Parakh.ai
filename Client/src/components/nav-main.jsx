              ...
              >
                <SidebarMenuItem>
                  <CollapsibleTrigger asChild>
                    <SidebarMenuButton 
                      tooltip={item.title}
                      className="cursor-pointer border border-gray-700/50 bg-gray-800/40 hover:bg-[#DFFF00] hover:text-black hover:border-[#DFFF00]/50 transition-all duration-200 font-medium text-sm rounded-md"
                      style={{ color: '#8d8e8f' }}
                    >
                      {item.icon && <item.icon className="h-4 w-4" />}
                      <span>{item.title}</span>
                      <ChevronRight className="chevron-right ml-auto h-4 w-4 transition-all duration-200 group-data-[state=open]/collapsible:rotate-90" />
                    </SidebarMenuButton>
                  </CollapsibleTrigger>
                  <CollapsibleContent>
                      ...
                      {item.items.map((subItem) => (
                        <SidebarMenuSubItem key={subItem.title}>
                          <SidebarMenuSubButton
                            ...
                            className="cursor-pointer text-sidebar-foreground/80 border border-gray-700/40 bg-gray-800/30 hover:bg-[#DFFF00] hover:text-black hover:border-[#DFFF00]/40 data-[state=active]:bg-[#DFFF00] data-[state=active]:text-black data-[state=active]:border-[#DFFF00] transition-all duration-200 font-medium text-xs rounded-sm group w-full"
                          >
                            <span className="cursor-pointer flex items-center gap-2">
                              {subItem.icon && (
                                <subItem.icon 
                                  className="h-3 w-3 transition-colors duration-200 group-hover:text-black"
                                  style={{ 
                                    color: location.pathname === subItem.url ? '#000000' : '#8d8e8f'
                                  }} 
                                />
                              )}
